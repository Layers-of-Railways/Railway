...
import static com.railwayteam.railways.registry.CRBlockPartials.WIDE_SCOTCH_WHEELS;

public class WideScotchYokeBogeyVisual implements BogeyVisual {
	...
	public WideScotchYokeBogeyVisual(VisualizationContext ctx, float partialTick, boolean inContraption) {
		wheel = ctx.instancerProvider()
				.instancer(InstanceTypes.TRANSFORMED, Models.partial(WIDE_SCOTCH_WHEELS))
				.createInstance();
		frame = ctx.instancerProvider()
				.instancer(InstanceTypes.TRANSFORMED, Models.partial(WIDE_DEFAULT_FRAME))
				.createInstance();
		pins = ctx.instancerProvider()
				.instancer(InstanceTypes.TRANSFORMED, Models.partial(WIDE_SCOTCH_PINS))
				.createInstance();
		pistons = ctx.instancerProvider()
				.instancer(InstanceTypes.TRANSFORMED, Models.partial(WIDE_SCOTCH_PISTONS))
				.createInstance();
