...
public class CRFluidsImpl {
    ...
    @Environment(EnvType.CLIENT)
    public static class PaintFluidVariantRenderHandler implements FluidVariantRenderHandler {
        @Override
        public int getColor(FluidVariant fluidVariant, @Nullable BlockAndTintGetter view, @Nullable BlockPos pos) {
            return PaintFluid.getColor(fluidVariant.getNbt())
                .map(PalettesColor::getDiffuseColor)
                .orElseGet(() -> FluidVariantRenderHandler.super.getColor(fluidVariant, view, pos)) | 0xff000000;
        }
    }
}
