package com.railwayteam.railways.misc;
...
import com.railwayteam.railways.interaction.capabilities.StationListCapability;
import com.railwayteam.railways.interaction.capabilities.StationListProvider;
...
public class RailwaysMinecartInteraction {
    @SubscribeEvent
    public void interactSpecificEntity(final PlayerInteractEvent.EntityInteractSpecific eis) {
        ...
        if (player.isSneaking()) {
            ...
            if (list.contains(candidate)) {
                if (eis.getSide().isClient())
                    player.sendStatusMessage(new StringTextComponent("station already assigned"), false);
                return;
            }
            list.add(candidate);
            ...
            if (eis.getSide().isClient())
                player.sendStatusMessage(new StringTextComponent("assigned station: " + candidate), false);
        }
        eis.setCanceled(true);
