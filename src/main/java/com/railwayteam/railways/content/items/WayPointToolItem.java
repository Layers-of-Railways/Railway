...
import com.railwayteam.railways.content.blocks.WayPointBlock;
...
import static com.railwayteam.railways.registry.CRBlocks.R_BLOCK_WAYPOINT;
...
public class WayPointToolItem extends Item {
	...
	private static final String MSG_RESET = "Selection reset.";
	private static final String MSG_VALID = "Valid connection with slope: "; // Y-delta is appended to this
	private static final String MSG_INVALID = "Invalid connection!";
	private static final String MSG_TOOLONG = "Track segment too long for one run!";
	private static final int MAX_TRACK_SEG_LEN = 64;

	public WayPointToolItem(Properties properties) {
		super(properties);
	}

	public static boolean isValid(BlockPos first, BlockPos second) {
		BlockPos diff = new BlockPos(second.getX() - first.getX(), second.getY() - first.getY(), second.getZ() - first.getZ());
				...
				if (!player.isCreative()) {
					player.world.addEntity(
					  new ItemEntity(player.world, step.getX(), step.getY(), step.getZ(),
							  new ItemStack(R_BLOCK_WAYPOINT.get(), 1))
					);
				}
			}
