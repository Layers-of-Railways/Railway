package com.railwayteam.railways.content.uiandrendering;
...
import static com.railwayteam.railways.registry.CRItems.R_ITEM_WAYPOINT_TOOL;
...
public class RailPathPlanRenderer {
  ...
  public static void onDrawBlockHighlightEvent(DrawHighlightEvent.HighlightBlock event) {
    ...
    if (!viewer.getHeldItem(Hand.MAIN_HAND).getItem().getName().equals(R_ITEM_WAYPOINT_TOOL.get().getName())) return;
    ...
    if (flag.hasTag() && flag.getTag().contains(WayPointToolItem.selectTag)) {
      ...
      //vs.forEachBox((x0, y0, z0, x1, y1, z1) -> {
        ...
        //all 4 of these lines are broken because m4f is never declared due to .peek() not having an easy replacement
